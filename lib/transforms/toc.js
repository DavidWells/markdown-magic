...
const { removeLeadingAndTrailingLineBreaks, escapeStringRegExp } = require('../utils/regex')
const { findMinIndent } = require('../utils/text')

module.exports = async function TOC(api) {
  // console.log("TOC API", api)
  const { options, block, currentContents, originalContents } = api
  const opts = options || {}
  ...
  if (opts.excludeText) {
    ...
    output = excludeTocItem(output, opts.excludeText)
    // // (\s+)?-(.*)\[Usage\]\(.*\)
  ...
  if (isSub) {
    // const start = currentContents.indexOf(open.value)
    // const linesBefore = currentContents.substr(0, start).split('\n')
    ...
    if (closestHeading) {
      ...
      const subItems = output.match(findSubToc)
      if (subItems) {
        ...
        if (finalItems) {
          ...
          output = finalItems.map((thing) => thing.substring(indent)).join('\n')
        } else {
...
function excludeTocItem(str, excludeText) {
  const matchTextEscaped = escapeStringRegExp(excludeText)
  /* (\s+)?-(.*)\[Usage\]\(.*\) */
