...
const cmd4 = stringToArgs('whatever=[uno] -v=false nice -funky no -word { foo: bar } -w yo --what nice -f -x -xyz false -u jimmy --testx --file **.md --u word')
...
test(`Handles values with equals "CLI ${cmd15.join(' ')}"`, () => {
  ...
  deepLog('result',result)
  assert.equal(result.mergedOptions, { param: [ 'userPoolId=us-west-1_fjsPJ6Q8J', 'userPoolClientId=19vdp5je9jsjkn488ddl4jvk19' ] })
})
